...
import { View, ScrollView, Picker, Alert, ActivityIndicator, ImageBackground } from 'react-native';
...
class Login extends React.PureComponent {
	...
	render() {
		...
		const serviceItems = supportedUILanguages.map((s) => (
			<Picker.Item key={s.iso_code} value={s.iso_code} label={s.name} />
		));
		return (
			<ImageBackground
				...
				<ScrollView style={loginStyles.scrollst}>
					...
					{isLoading && <ActivityIndicator size={100} color="#ABC1DE" />}
				</ScrollView>
			</ImageBackground>
		);
	}
