...
class CustomerList extends React.Component {
		...
		if (type === 'NORMAL' && data.item.id != null) {
			return (
				<TouchableHighlight
					...
					<View style={[this.getRowBackground(index, isSelected), styles.listrowcont]}>
						...
						<View style={styles.flexOne}>
							<Text style={styles.baseItem}>{data.item.phoneNumber}</Text>
						</View>
						...
						<View style={styles.flexOne}>
							<Text style={styles.baseItem}>{data.item.customerType}</Text>
						</View>
						<View style={styles.flexOne}>
							<Text style={styles.baseItem}>{data.item.dueAmount.toFixed(2)}</Text>
						</View>
						<View style={styles.flexOne}>
	...
	_isAnonymousCustomer(customer) {
		return CustomerTypeRealm.getCustomerTypeByName('anonymous').id == customer.customerTypeId;
	}

	deleteCustomer() {
		const alertMessage = i18n.t('delete-specific-customer', {
