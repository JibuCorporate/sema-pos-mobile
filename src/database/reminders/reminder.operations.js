...
class ReminderRealm {
  ...
  updateReminder(reminder) {
    try {
      realm.write(() => {
        const reminderObj = realm.objects('Reminder').filtered(`id = "${reminder.reminder_id}"`);
  ...
  resetSelected() {
    try {
      realm.write(() => {
        const reminderObj = realm.objects('Reminder');
  ...
  hardDeleteReminder(reminder) {
    try {
      realm.write(() => {
        const reminders = realm.objects('Reminder');
        const deleteReminder = reminders.filtered(`id = "${reminder.reminder_id}"`);
