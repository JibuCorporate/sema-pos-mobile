...
class CreditRealm {
  ...
  truncate() {
    try {
      realm.write(() => {
        const credits = realm.objects('Credit');
  ...
  setLastCreditSync() {
    realm.write(() => {
      const syncDate = realm.objects('CreditSyncDate');
  ...
  getAllCreditByDate(date) {
    const credit = Object.values(JSON.parse(JSON.stringify(realm.objects('Credit'))));
    return credit.filter((r) => compareAsc(parseISO(r.created_at), parseISO(date)) === 1 || compareAsc(parseISO(r.updated_at), parseISO(date)) === 1);
  }
  ...
  updateCredit(credit) {
    try {
      realm.write(() => {
        const creditObj = realm.objects('Credit').filtered(`top_up_id = "${credit.top_up_id}"`);
  ...
  getCreditByRecieptId(receipt_id) {
    const credit = Object.values(JSON.parse(JSON.stringify(realm.objects('Credit').filtered(`receipt_id = "${receipt_id}"`))));
    return credit[0];
  }
  ...
  hardDeleteCredit(credit) {
    try {
      realm.write(() => {
        const credits = realm.objects('Credit');
        const deleteCredit = credits.filtered(`top_up_id = "${credit.top_up_id}"`);
      ...
      try {
        ...
        realm.write(() => {
          for (i = 0; i < credit.length; i++) {
            const ischeckCredit = this.checkCredit(credit[i].created_at, credit[i].top_up_id).length;
            if (ischeckCredit === 0) {
              const value = realm.create('Credit', {
                ...credit[i],
              ...
              const discountObj = realm.objects('Credit').filtered(`top_up_id = "${credit[i].top_up_id}"`);
