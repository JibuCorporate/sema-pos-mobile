...
class DiscountRealm {
  ...
  truncate() {
    try {
      realm.write(() => {
        const discounts = realm.objects('Discount');
  ...
  setLastDiscountSync() {
    realm.write(() => {
      const syncDate = realm.objects('DiscountSyncDate');
      syncDate[0].lastDiscountSync = new Date();
    });
  }

  geDiscountsByDate(date) {
    const discounts = Object.values(JSON.parse(JSON.stringify(realm.objects('Discount'))));
    return discounts.filter((r) => compareAsc(parseISO(r.created_at), parseISO(date)) === 1 || compareAsc(parseISO(r.updated_at), parseISO(date)) === 1 || r.active === false);
  }
  ...
  updateDiscount(discount) {
    try {
      realm.write(() => {
        const discountObj = realm.objects('Discount').filtered(`id = "${discount.id}"`);
  ...
  resetSelected() {
    try {
      realm.write(() => {
        const discountObj = realm.objects('Discount');
  ...
  hardDeleteDiscount(discount) {
    try {
      realm.write(() => {
        const discounts = realm.objects('Discount');
        const deleteDiscount = discounts.filtered(`id = "${discount.id}"`);
      ...
      try {
        ...
        realm.write(() => {
          for (i = 0; i < discounts.length; i++) {
            const ischeckdiscounts = this.checkdiscounts(discounts[i].created_at, discounts[i].id).length;
            if (ischeckdiscounts === 0) {
              const value = realm.create('Discount', {
                ...discounts[i],
              ...
              const discountObj = realm.objects('Discount').filtered(`id = "${discounts[i].id}"`);
