...
class ProductMRPRealm {
  ...
  truncate() {
    try {
      realm.write(() => {
        const productMRPs = realm.objects('ProductMRP');
  ...
  setLastProductMRPSync() {
    realm.write(() => {
      const syncDate = realm.objects('ProductMRPSyncDate');
      syncDate[0].lastProductMRPSync = new Date();
    });
  }

  getFilteredProductMRP() {
    const productMrpDict = {}; // Note - This assumes that all productMrps are being saved
    const productMrpsArray = Object.values(JSON.parse(JSON.stringify(realm.objects('ProductMRP').filtered(`active = ${true}`))));
  ...
  getProductMRPSByDate(date) {
    const productMrps = Object.values(JSON.parse(JSON.stringify(realm.objects('ProductMRP'))));
    return productMrps.filter((r) => compareAsc(parseISO(r.created_at), parseISO(date)) === 1 || compareAsc(parseISO(r.updated_at), parseISO(date)) === 1 || r.active === false);
  }
  ...
  createProductMRP(kiosk_id, product_id, quantity, filterDate) {
    const existingProductMRP = this.getProductMRPS().filter((productMRP) => this.formatDay(productMRP.created_at) === this.formatDay(filterDate) && productMRP.product_id === product_id);
    const now = new Date();
    ...
    if (existingProductMRP.length > 0) {
      return this.updateProductMRP(
        {
          ...existingProductMRP[0], quantity, updated_at: now, syncAction: 'update',
        },
      );
    }
  }
  ...
  updateProductMRP(productMRP) {
    try {
      realm.write(() => {
        const productMRPObj = realm.objects('ProductMRP').filtered(`id = "${productMRP.id}"`);
  ...
  hardDeleteProductMRP(productMRP) {
    try {
      realm.write(() => {
        const productMRPs = realm.objects('ProductMRP');
        const deleteProductMRP = productMRPs.filtered(`id = "${productMRP.id}"`);
      ...
      try {
        ...
        realm.write(() => {
          for (i = 0; i < productMRPs.length; i++) {
            const ischeckproductMRPs = this.checkProductMrps(productMRPs[i].created_at, productMRPs[i].id).length;
            if (ischeckproductMRPs === 0) {
              const value = realm.create('ProductMRP', {
                ...productMRPs[i],
              ...
              const discountObj = realm.objects('ProductMRP').filtered(`id = "${productMRPs[i].id}"`);
