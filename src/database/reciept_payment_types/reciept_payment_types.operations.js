...
class ReceiptPaymentTypeRealm {
  ...
  setLastReceiptPaymentTypeSync() {
    realm.write(() => {
      const syncDate = realm.objects('ReceiptPaymentTypeSyncDate');
  ...
  truncate() {
    try {
      realm.write(() => {
        const receiptPaymentTypes = realm.objects('ReceiptPaymentType');
  ...
  getReceiptPaymentTypesByDate(date) {
    try {
      const orderObj = Object.values(JSON.parse(JSON.stringify(realm.objects('ReceiptPaymentType'))));
      const orderObj2 = orderObj.map(
        (item) => ({
          ...item,
      ...
      return orderObj2.filter((r) => compareAsc(parseISO(r.created_at), parseISO(date)) === 1 || compareAsc(parseISO(r.updated_at), parseISO(date)) === 1);
    } catch (e) {
    }
  }
  ...
  updateReceiptPaymentType(receiptPaymentType) {
    try {
      realm.write(() => {
        const receiptPaymentTypeObj = realm.objects('ReceiptPaymentType').filtered(`receipt_payment_type_id = "${receiptPaymentType.receipt_payment_type_id}"`);
  ...
  resetSelected() {
    try {
      realm.write(() => {
        const receiptPaymentTypeObj = realm.objects('ReceiptPaymentType');
  ...
  isSelected(receiptPaymentType, isSelected) {
    try {
      realm.write(() => {
        const receiptPaymentTypeObj = realm.objects('ReceiptPaymentType').filtered(`id = "${receiptPaymentType.id}"`);
  ...
  hardDeleteReceiptPaymentType(receiptPaymentType) {
    try {
      realm.write(() => {
        const receiptPaymentTypes = realm.objects('ReceiptPaymentType');
        const deleteReceiptPaymentType = receiptPaymentTypes.filtered(`receipt_payment_type_id = "${receiptPaymentType.receipt_payment_type_id}"`);
        ...
        if (receiptId) {
          receiptPaymentTypes.forEach((obj) => {
            realm.create('ReceiptPaymentType', {
              ...
              created_at: receiptDate || obj.created_at,
      ...
      try {
        ...
        realm.write(() => {
          for (i = 0; i < receiptPaymentTypes.length; i++) {
            const ischeckReceiptPaymentType = this.checkReceiptPaymentType(receiptPaymentTypes[i].created_at, receiptPaymentTypes[i].receipt_payment_type_id).length;
            if (ischeckReceiptPaymentType === 0) {
              const value = realm.create('ReceiptPaymentType', {
              ...
              const receiptPaymentTypeUpdate = realm.objects('ReceiptPaymentType').filtered(`receipt_payment_type_id = "${receiptPaymentTypes[i].receipt_payment_type_id}"`);
