...
class OrderRealm {
  ...
  setLastOrderSync() {
    realm.write(() => {
      const syncDate = realm.objects('OrderSyncDate');
  ...
  getAllOrder() {
    const formattedArray = [...Object.values(JSON.parse(JSON.stringify(realm.objects('Order'))))];
    for (const i in formattedArray) {
      ...
      formattedArray[i].receipt_line_items = JSON.parse(formattedArray[i].receipt_line_items).map(
        (e) => ({
          ...e, product: JSON.parse(JSON.stringify(realm.objects('Product').filtered(`productId = "${e.product_id}"`)))[0],
        }),
      );
    }
  ...
  getActiveOrders() {
    const formattedArray = [...Object.values(
  ...
  getOrdersByDate(date) {
    return new Promise((resolve) => {
      try {
        const orderObj = Object.values(JSON.parse(JSON.stringify(realm.objects('Order'))));
  ...
  getOrdersByDate2(date) {
    try {
      ...
      // return r.created_at === format(parseISO(date), 'yyyy-MM-dd') || r.updated_at === format(parseISO(date), 'yyyy-MM-dd')
        compareAsc(parseISO(r.created_at), parseISO(date)) === 1 || compareAsc(parseISO(r.updated_at), parseISO(date)) === 1 || r.active === false);
    } catch (e) {
    }
  }
  ...
  createOrder(receipt) {
    ...
    for (const i in receipt.products) {
  ...
  updateOrder(
    ...
    try {
      realm.write(() => {
        const orderObj = realm.objects('Order').filtered(`orderId = "${order.orderId}"`);
  ...
  synched(order) {
    try {
      realm.write(() => {
        const orderObj = realm.objects('Order').filtered(`receiptId = "${order.receiptId}"`);
  ...
  hardDeleteOrder(order) {
    try {
      realm.write(() => {
        const orders = realm.objects('Order');
        const deleteOrder = orders.filtered(`receiptId = "${order.receiptId}"`);
      ...
      try {
        ...
        realm.write(() => {
          for (i = 0; i < orders.length; i++) {
            const ischeckOrder = this.checkOrder(orders[i].created_at, orders[i].uuid).length;
            if (ischeckOrder === 0) {
              const value = realm.create('Order', {
                ...orders[i],
              ...
              const orderObj = realm.objects('Order').filtered(`uuid = "${orders[i].uuid}"`);
