...
class SalesChannelRealm {
  ...
  setLastSalesChannelSync() {
    realm.write(() => {
      const syncDate = realm.objects('SalesChannelSyncDate');
  ...
  getSalesChannelsByDate(date) {
    const salesChannels = Object.values(JSON.parse(JSON.stringify(realm.objects('SalesChannel'))));
    return salesChannels.filter((r) => compareAsc(parseISO(r.created_at), parseISO(date)) === 1 || compareAsc(parseISO(r.updated_at), parseISO(date)) === 1 || r.active === false);
  }
      ...
      try {
        ...
        realm.write(() => {
          for (i = 0; i < salesChannels.length; i++) {
            const ischeckSalesChannels = this.checkSalesChannels(salesChannels[i].created_at, salesChannels[i].id).length;
            if (ischeckSalesChannels === 0) {
              const value = realm.create('SalesChannel', { ...salesChannels[i], active: true });
              ...
              const salesChannelObj = realm.objects('SalesChannel').filtered(`id = "${salesChannels[i].id}"`);
