...
class CustomerRealm {
  constructor() {
    this.customer = [];
    const firstSyncDate = format(new Date('2017-01-01'), 'yyyy-MM-dd');
    realm.write(() => {
  ...
  setLastCustomerSync() {
    realm.write(() => {
      const syncDate = realm.objects('CustomerSyncDate');
  ...
  getAllCustomer() {
    ...
    return customers.filter((r) => r.is_delete === null || r.is_delete === 1);
  }

  selectedCustomer(customerId) {
    realm.write(() => {
      const customers = realm.objects('Customer').filtered(`customerId = "${customerId}"`);
  ...
  getCustomerById(customerId) {
    const customers = Object.values(JSON.parse(JSON.stringify(realm.objects('Customer').filtered(`customerId = "${customerId}"`))));
    return customers[0];
  }

  getCustomerBycreated_at(date) {
    try {
      const orderObj = Object.values(JSON.parse(JSON.stringify(realm.objects('Customer'))));
      return orderObj.filter((r) => compareAsc(parseISO(r.created_at), parseISO(date)) === 1 || compareAsc(parseISO(r.updated_at), parseISO(date)) === 1);
    } catch (e) {
    }
  }
  ...
  updateCustomer(
    ...
    return new Promise((resolve, reject) => {
      try {
        realm.write(() => {
          const customerObj = realm.objects('Customer').filtered(`customerId = "${customer.customerId}"`);
  ...
  hardDeleteCustomer(customer) {
    try {
      realm.write(() => {
        const customers = realm.objects('Customer');
        const deleteCustomer = customers.filtered(`customerId = "${customer.customerId}"`);
      ...
      try {
        ...
        realm.write(() => {
          for (i = 0; i < customers.length; i++) {
            const ischeckCustomer = this.checkCustomer(customers[i].created_at, customers[i].customerId).length;
            if (ischeckCustomer === 0) {
              const value = realm.create('Customer', {
                ...customers[i],
              ...
              const customerObj = realm.objects('Customer').filtered(`customerId = "${customers[i].id}"`);
