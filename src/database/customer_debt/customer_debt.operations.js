...
class CustomerDebtRealm {
  ...
  setLastCustomerDebtSync() {
    realm.write(() => {
      const syncDate = realm.objects('CustomerDebtSyncDate');
  ...
  truncate() {
    try {
      realm.write(() => {
        const customerDebts = realm.objects('CustomerDebt');
  ...
  getCustomerDebtsByDate(date) {
    const customerDebt = Object.values(JSON.parse(JSON.stringify(realm.objects('CustomerDebt'))));
    return customerDebt.filter((r) => compareAsc(parseISO(r.created_at), parseISO(date)) === 1 || compareAsc(parseISO(r.updated_at), parseISO(date)) === 1);
  }
  ...
  updateCustomerDebt(customerDebt) {
    try {
      realm.write(() => {
        const customerDebtObj = realm.objects('CustomerDebt').filtered(`id = "${customerDebt.customer_debt_id}"`);
  ...
  resetSelected() {
    try {
      realm.write(() => {
        const customerDebtObj = realm.objects('CustomerDebt');
  ...
  isSelected(customerDebt, isSelected) {
    try {
      realm.write(() => {
        const customerDebtObj = realm.objects('CustomerDebt').filtered(`customer_debt_id = "${customerDebt.customer_debt_id}"`);
  ...
  hardDeleteCustomerDebt(customerDebt) {
    try {
      realm.write(() => {
        const customerDebts = realm.objects('CustomerDebt');
        const deleteCustomerDebt = customerDebts.filtered(`customer_debt_id = "${customerDebt.customer_debt_id}"`);
  ...
  getCustomerDebtByRecieptId(receipt_id) {
    const customerDebt = Object.values(JSON.parse(JSON.stringify(realm.objects('CustomerDebt').filtered(`receipt_id = "${receipt_id}"`))));
      ...
      try {
        ...
        realm.write(() => {
          for (i = 0; i < customerDebts.length; i++) {
            const ischeckCustomerDebt = this.checkCustomerDebt(customerDebts[i].created_at, customerDebts[i].customer_debt_id).length;
            if (ischeckCustomerDebt === 0) {
              const value = realm.create('CustomerDebt', {
                ...customerDebts[i],
                synched: true,
              ...
              const customerDebtUpdate = realm.objects('CustomerDebt').filtered(`customer_debt_id = "${customerDebts[i].customer_debt_id}"`);
