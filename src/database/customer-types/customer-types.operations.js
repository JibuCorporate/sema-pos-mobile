...
class CustomerTypeRealm {
  ...
  setLastCustomerTypesSync() {
    realm.write(() => {
      const syncDate = realm.objects('CustomerTypesSyncDate');
  ...
  getCustomerTypesByDate(date) {
    const customerTypes = Object.values(JSON.parse(JSON.stringify(realm.objects('CustomerType'))));
    return customerTypes.filter((r) => compareAsc(parseISO(r.created_at), parseISO(date)) === 1 || compareAsc(parseISO(r.updated_at), parseISO(date)) === 1 || r.active === false);
  }

  getCustomerTypesForDisplay(salesChannelId = 0) {
    const customerTypesForDisplay = [];
    if (salesChannelId != 0) {
      ...
      try {
        ...
        realm.write(() => {
          for (i = 0; i < customerTypes.length; i++) {
            const ischeckcustomerTypes = this.checkcustomerTypes(customerTypes[i].created_at, customerTypes[i].id).length;
            if (ischeckcustomerTypes === 0) {
              const value = realm.create('CustomerType', {
                ...customerTypes[i],
              ...
              const customerTypeObj = realm.objects('CustomerType').filtered(`id = "${customerTypes[i].id}"`);
