import {
  parseISO, format, sub, add, isValid, compareAsc,
} from 'date-fns';
...
class CustomerReminderRealm {
  constructor() {
    ...
    const firstSyncDate = format(sub(new Date(), { days: 30 }), 'yyyy-MM-dd');
    realm.write(() => {
  ...
  truncate() {
    try {
      realm.write(() => {
        const customerReminders = realm.objects('CustomerReminder');
        const customerRemindersSync = realm.objects('CustomerReminderSyncDate');
  ...
  synchedCustomerReminder(customerReminder) {
    try {
      realm.write(() => {
        const customerReminderObj = realm.objects('CustomerReminder').filtered(`reminder_id = "${customerReminder.reminder_id}"`);
  ...
  setLastCustomerReminderSync() {
    realm.write(() => {
      const syncDate = realm.objects('CustomerReminderSyncDate');
  ...
  deleteCustomerReminder(reminder_id) {
    try {
      realm.write(() => {
        const customerReminderObj = realm.objects('CustomerReminder').filtered(`reminder_id = "${reminder_id}"`);
  ...
  getAllCustomerReminderByDate(date) {
    const customerReminder = Object.values(JSON.parse(JSON.stringify(realm.objects('CustomerReminder'))));
    return customerReminder.filter((r) => compareAsc(parseISO(r.created_at), parseISO(date)) === 1 || compareAsc(parseISO(r.updated_at), parseISO(date)) === 1 || r.active === false);
  }

  getCustomerReminders() {
    return Object.values(JSON.parse(JSON.stringify(realm.objects('CustomerReminder')))).map((e) => ({ ...e, customer: CustomerRealm.getCustomerById(e.customer_account_id) }));
  }
  ...
  createCustomerReminder(customerReminder, kiosk_id) {
    try {
      realm.write(() => {
        const customerReminderObj = Object.values(JSON.parse(JSON.stringify(realm.objects('CustomerReminder').filtered(`customer_account_id = "${customerReminder.customer_account_id}"`))));

        if (customerReminderObj.length > 0) {
          const customerReminderUpdateObj = realm.objects('CustomerReminder').filtered(`customer_account_id = "${customerReminder.customer_account_id}"`);
  ...
  setCustomReminder(customer_account_id, custom_reminder_date) {
    try {
      realm.write(() => {
        const customerReminderObj = Object.values(JSON.parse(JSON.stringify(realm.objects('CustomerReminder').filtered(`customer_account_id = "${customer_account_id}"`))));
        if (customerReminderObj.length > 0) {
          const customerReminderUpdateObj = realm.objects('CustomerReminder').filtered(`customer_account_id = "${customer_account_id}"`);
  ...
  updateCustomerReminder(customerReminder) {
    try {
      realm.write(() => {
        const customerReminderObj = realm.objects('CustomerReminder').filtered(`id = "${customerReminder.customerReminderId}"`);
  ...
  resetSelected() {
    try {
      realm.write(() => {
        const customerReminderObj = realm.objects('CustomerReminder');
  ...
  hardDeleteCustomerReminder(customerReminder) {
    try {
      realm.write(() => {
        const customerReminders = realm.objects('CustomerReminder');
        const deleteCustomerReminder = customerReminders.filtered(`id = "${customerReminder.customerReminderId}"`);
        ...
        realm.write(() => {
          for (i = 0; i < customerReminders.length; i++) {
            if (this.checkCustomerReminderDate(customerReminders[i].created_at).length === 0) {
              const value = realm.create('CustomerReminder', { ...customerReminders[i], active: true });
