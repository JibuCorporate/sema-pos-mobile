...
class ProductsRealm {
  ...
  setLastProductsync() {
    realm.write(() => {
      const syncDate = realm.objects('ProductSyncDate');
  ...
  getProductsByDate(date) {
    try {
      const products = Object.values(JSON.parse(JSON.stringify(realm.objects('Product'))));
      return products.filter((r) => compareAsc(parseISO(r.created_at), parseISO(date)) === 1 || compareAsc(parseISO(r.updated_at), parseISO(date)) === 1 || r.active === false);
    } catch (e) {
    }
  }
  ...
  createProducts(kiosk_id, product_id, quantity, filterDate) {
    const existingProducts = this.getProducts().filter((product) => this.formatDay(product.created_at) === this.formatDay(filterDate) && product.product_id === product_id);
    const now = new Date();
    ...
    if (existingProducts.length > 0) {
      return this.updateProducts(
        {
          ...existingProducts[0], quantity, updated_at: now, syncAction: 'update',
        },
      );
    }
  }
  ...
  updateProducts(product) {
    try {
      realm.write(() => {
        const productObj = realm.objects('Product').filtered(`productId = "${product.productId}"`);
  ...
  hardDeleteProducts(product) {
    try {
      realm.write(() => {
        const products = realm.objects('Product');
        const deleteProducts = products.filtered(`productId = "${product.productId}"`);
      ...
      try {
        ...
        realm.write(() => {
          for (i = 0; i < products.length; i++) {
            const ischeckproducts = this.checkProduct(products[i].created_at, products[i].id).length;
            if (ischeckproducts === 0) {
              const value = realm.create('Product', {
                ...products[i],
              ...
              const discountObj = realm.objects('Product').filtered(`id = "${products[i].id}"`);
