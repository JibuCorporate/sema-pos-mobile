...
class Communications {
  ...
  initialize(url, site, user, password, token, siteId) {
    ...
    const produrl = 'http://142.93.115.206:3002/';
    this._url = produrl;
    this._site = site;
    this._user = user;
  ...
  getReminders() {
    ...
    const urlr = `sema/reminders?site-id=${this._siteId}`;
    that = this;
    return fetch(that._url + urlr, options).then((response) => response.json()).catch((error) => {});
  }
}
