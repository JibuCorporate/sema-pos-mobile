import * as _ from 'lodash';
import ProductMRPRealm from '../../database/productmrp/productmrp.operations';
import ProductApi from '../api/product.api';
import SyncUtils from './syncUtils';

class ProductMRPSync {
  synchronizeProductMrps(regionId) {
    return new Promise((resolve) => {
      ProductApi.getProductMrps(regionId, ProductMRPRealm.getLastProductMRPSync())
        .then(async (remoteProductMRP) => {
          try {
            const initlocalProductMRPs = ProductMRPRealm.getProductMRPSByDate(ProductMRPRealm.getLastProductMRPSync());
            const localProductMRPs = initlocalProductMRPs.length > 0 ? [...initlocalProductMRPs] : [];
            const remoteProductMRPs = remoteProductMRP.pricing.length > 0 ? [...remoteProductMRP.pricing] : [];

            const onlyInLocal = localProductMRPs.filter(SyncUtils.compareRemoteAndLocal(remoteProductMRPs, 'id'));
            const onlyInRemote = remoteProductMRPs.filter(SyncUtils.compareRemoteAndLocal(localProductMRPs, 'id'));

            const syncResponseArray = [];
            if (onlyInLocal.length > 0) {
              for (const property in onlyInLocal) {

              }
            }

            if (onlyInRemote.length > 0) {
              const localResponse = await ProductMRPRealm.createManyProductMRP(onlyInRemote);
              syncResponseArray.push(...localResponse);
              ProductMRPRealm.setLastProductMRPSync();
            }

            resolve({
              success: syncResponseArray.length > 0 ? syncResponseArray[0].status : 'success',
              productMrps: onlyInLocal.concat(onlyInRemote).length,
              successError: syncResponseArray.length > 0 ? syncResponseArray[0].status : 'success',
              successMessage: syncResponseArray.length > 0 ? syncResponseArray[0] : 'success',
            });
          } catch (error) {
            console.log('error', error);
          }
        })
        .catch((error) => {
          resolve({
            error: true,
            productMrps: 0,
          });
        });
    });
  }
}
export default new ProductMRPSync();
