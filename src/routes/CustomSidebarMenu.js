...
import {
  View, StyleSheet, Image, Text, Alert, ActivityIndicator, ScrollView, TouchableOpacity,
} from 'react-native';
...
const txtStyle = (key, index) => StyleSheet.create({
  txtCol: {
    fontSize: 15,
    color: index === key ? colors.red : colors.black,
  },
  drawerSty: {
    flex: 1,
    flexDirection: 'row',
    ...
    paddingTop: 10,
    paddingBottom: 10,
    backgroundColor: index === key ? colors.gray : colors.white,
  },
});
...
class CustomSidebarMenu extends React.Component {
  ...
  static getSyncResults(syncResult) {
    try {
      if (
      ...
      if (syncResult.customers.customers > 0) {
        final1 = `${syncResult.customers.successError === 'success'
      ...
      if (syncResult.wastageReport.wastageReport > 0) {
        final3 = `\n${syncResult.wastageReport.successError === 'success'
          ? `${syncResult.wastageReport.wastageReport} ${i18n.t('wastageReport-updated')}`
      ...
      if (
        ...
        final6 = `\n${syncResult.meterReading.successError === 'success'
          ? `${syncResult.meterReading.meterReading} ${i18n.t('meterReading-updated')}`
      ...
      if (
        ...
        final7 = `\n${syncResult.recieptPayments.successError === 'success'
          ? `${syncResult.recieptPayments.recieptPayments} ${i18n.t('recieptPayments-updated')}`
      ...
      if (
        ...
        final8 = `\n${syncResult.customerReminder.successError === 'success'
          ? `${syncResult.customerReminder.customerReminder} ${i18n.t('customer-reminder-updated')}`
      ...
      if (
        ...
        final10 = `\n${syncResult.salesChannels.successError === 'success'
          ? `${syncResult.salesChannels.salesChannels} ${i18n.t('salechannel-updated')}`
    ...
    return (
      <View style={customSiderBarStyles.sideMenuContainer}>
        <ScrollView style={customSiderBarStyles.viewFlex}>
          <Image source={logo} resizeMode="stretch" style={customSiderBarStyles.imageStyle} />
          ...
          <View style={customSiderBarStyles.viewFlex}>
            {this.items.map((item, key) => (
              <View style={customSiderBarStyles.viewFlex} key={item.navOptionName}>
                <TouchableOpacity
                  style={txtStyle(key, global.currentScreenIndex).drawerSty}
