...
comparePaymentTypeReceipts = () => {
  ...
  const customerReceipts = OrderRealm.getAllOrder();
...
comparePaymentTypes = () => {
  const receiptsPaymentTypes = ReceiptPaymentTypeRealm.getReceiptPaymentTypes();
...
prepareData = () => {
  ...
  receipts.sort((a, b) => (isBefore(new Date(a.createdAt), new Date(b.createdAt))
    ? 1
    : -1));
...
prepareCustomerDebt = () => {
  ...
  debtPayment.sort((a, b) => (isBefore(new Date(a.createdAt), new Date(b.createdAt))
    ? 1
    : -1));
  return [...debtPayment];
};
...
prepareTopUpData = () => {
  ...
  topups.sort((a, b) => (isBefore(new Date(a.createdAt), new Date(b.createdAt))
    ? 1
    : -1));
  return [...topups];
};

groupBySectionTitle = (objectArray, property) => objectArray.reduce((acc, obj) => {
...
export function prepareSectionedData() {
  ...
  const finalArray = (deptPayment.concat(topups)).concat(receipts).sort((a, b) => (isBefore(new Date(a.createdAt), new Date(b.createdAt))
    ? 1
    : -1));
