...
const initialState = {
  products: [], channel: { salesChannel: 'direct' }, flow: { page: 'products' }, payment: { cash: 0, credit: 0, mobile: 0 }, discounts: [],
};
...
const orderReducer = (state = initialState, action) => {
  ...
  switch (action.type) {
    ...
    case ADD_PRODUCT_TO_ORDER:
      ...
      if (index !== -1) {
        const updateProducts = newState.products.filter((u) => u.product.productId !== newState.products[index].product.productId);
    ...
    case SET_DISCOUNTS:
      newState = { ...state };
      // Check if product exists
      for (const product of newState.discounts) {
        if (product.product.productId === action.data.product.productId) {
    ...
    case REMOVE_PRODUCT_DISCOUNT:
      newState = { ...state };
      newState.discounts = [];
      for (const product of state.discounts) {
        if (product.product.productId !== action.data.product.productId) {
